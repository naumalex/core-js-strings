...
function isPalindrome(str) {
  const strNormalized = str
    ...
    .replaceAll('!', '')
    .replaceAll('?', '');
  const reversStr = strNormalized.split('').reverse().join('');
  return strNormalized.toUpperCase() === reversStr.toUpperCase();
}
...
function getCardId(value) {
  ...
  const row = obj[value[value.length - 1]];
  const firstChars = value.substring(0, value.length - 1);
  obj = {
    A: 1,
    J: 11,
    Q: 12,
  ...
  const col = parseInt(obj[firstChars], 10) || parseInt(firstChars, 10);
  return 13 * row + col - 1;
}
